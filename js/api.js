(function (global) {
  ...
  function normalizeResult(result) {
    return { game_id: String(result.game_id || '').trim(), date: result.date || '', player_id: String(result.player_id || '').trim(), player_name: String(result.player_name || '').trim(), score: Number(result.score) || 0, rank: Number(result.rank) || 0, seat_order: Number(result.seat_order) || 0, yakitori: result.yakitori === true || String(result.yakitori).toLowerCase() === 'true', point: Number(result.point) || 0 };
  }
