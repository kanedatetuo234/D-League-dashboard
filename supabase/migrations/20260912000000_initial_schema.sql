...
-- Preserve the exact rule settings used when a match was registered.
...
-- Keep an audit trail for match registration, edits, and deletion requests.
...
create index if not exists match_change_logs_match_idx on public.match_change_logs (match_id, changed_at desc);
create index if not exists match_change_logs_changed_at_idx on public.match_change_logs (changed_at desc);
...
create policy "public can read match rule snapshots" on public.match_rule_snapshots for select using (true);
